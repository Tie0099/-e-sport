...
import styles from '@/styles/TournamentBracket32.module.css';

const TournamentBracket32 = ({ matches = {}, handleDrop, handleDragStartFromBracket, handleScoreChange }) => {
  ...
  return (
    <div className={styles.tournament}>
          ...
          <div key={num} className={styles[`Pair2_${num}`]}>
            <div>
              ...
              <input
                type="text"
                placeholder="ผล"
                value={matches[`round${num}`]?.score1 || ''}
                onChange={(e) => handleInputFilter(e, `round${num}`, 'score1', matches[`round${num}`]?.score2)}
                className={styles.score}
              />
            </div>
            <div>
              ...
              <input
                type="text"
                placeholder="ผล"
                value={matches[`round${num}`]?.score2 || ''}
                onChange={(e) => handleInputFilter(e, `round${num}`, 'score2', matches[`round${num}`]?.score1)}
                className={styles.score}
              />
            </div>
          </div>
      ...
      {/* รอบ 8 ทีม (Quarterfinal) */}
      <div className={styles.Quarterfinal}>
        {[1, 2, 3, 4].map((num) => (
      ...
      <div className={styles.Semifinal}>
        {[1, 2].map((num) => (
          <div key={num} className={styles[`Pair4_${num}`]}>
            <div>
              <input
                type="text"
                placeholder="ชื่อทีม"
