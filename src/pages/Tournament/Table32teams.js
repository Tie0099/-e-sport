...
const MatchTable32Teams = () => {
  ...
  return (
    <div className={styles.container}>
      <h1>โปรแกรมการแข่งขัน รอบ 32 ทีม</h1>
      <table className={styles.table}>
        ...
        <tbody>
          {[...Array(16)].map((_, index) => {
            const match = currentMatches[`match${index + 1}`] || {};
            return (
              <tr key={index}>
                ...
                <td className={styles.cell}>
                  <DatePicker
                    ...
                    onChange={(date) => handleInputChange('date', `match${index + 1}`, date.toISOString())}
                ...
                <td className={styles.cell}>
                  <TimePicker
                    onChange={(time) => handleInputChange('time', `match${index + 1}`, time)}
                    value={match.time || ''}
                ...
                <td className={styles.cell}>
                  <input
                    ...
                    value={match.notes || ''}
                    onChange={(e) => handleInputChange('notes', `match${index + 1}`, e.target.value)}
                  />
                </td>
              </tr>
            );
...
export default MatchTable32Teams;
